...
class FolderTree(QWidget):
    ...
    selection_changed = pyqtSignal(list)   # list[str] đường dẫn đã tick
    file_removed = pyqtSignal(str)         # đường dẫn file vừa bị xóa khỏi danh sách
    ...
    def set_files(self, files: list[ScannedFile], counts: dict[str, int]):
        ...
        for group in order:
            ...
            if group:
                ...
                header.toggled.connect(lambda g=group: self._apply_visibility(g))
    ...
    def _make_chip(self, label: str, ext_type: str | None) -> QPushButton:
        ...
        chip.clicked.connect(lambda _, t=ext_type: self._set_filter(t))
