...
class ChatPanel(QFrame):
    ...
    file_card_clicked = pyqtSignal(str)
    ...
    def set_chips(self, labels: list[str]):
        ...
        for i, label in enumerate(labels):
            ...
            chip.clicked.connect(lambda _, t=label: self.send_text(t))
            self._chips_lay.insertWidget(i, chip)
    ...
    def add_file_card(self, file_name: str, badge: str = "XLS"):
        card = QPushButton(f"[{badge}]  {file_name}")
        card.setProperty("class", "fileCard")
        card.setCursor(Qt.CursorShape.PointingHandCursor)
        card.clicked.connect(lambda: self.file_card_clicked.emit(file_name))
