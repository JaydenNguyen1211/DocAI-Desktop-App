...
class Sidebar(QFrame):
    ...
    recent_deleted = pyqtSignal(str)  # bấm xóa 1 item trong danh sách "Gần đây"

    open_folder = pyqtSignal()             # bấm "Mở thư mục làm việc" (trống)
    change_folder = pyqtSignal()           # bấm "Đổi" (đang quét / sẵn sàng)
    folder_selection_changed = pyqtSignal(list)   # list[str] file đã tick
    folder_file_removed = pyqtSignal(str)         # bấm xóa 1 file trong cây thư mục
    ...
    def __init__(self, parent=None):
        ...
        for key, label in (("file", "Tệp"), ("folder", "Thư mục")):
            ...
            if key == "file":
                btn.setChecked(True)
            btn.clicked.connect(lambda _, k=key: self._on_seg_clicked(k))
