...
class CentralChat(QFrame):
    ...
    message_sent = pyqtSignal(str)   # gõ lệnh / bấm chip → bắt đầu trò chuyện
    file_chosen = pyqtSignal(str)    # đính kèm / kéo-thả file → vào workspace
    ...
    def __init__(self, parent=None):
        ...
        row = None
        for i, (label, color) in enumerate(EMPTY_CHIPS):
            if i % 2 == 0:
                row = QHBoxLayout()
                row.setSpacing(8)
                row.addStretch()
            ...
            chip.clicked.connect(lambda _, t=label: self.message_sent.emit(t))
            row.addWidget(chip)
            if i % 2 == 1 or i == len(EMPTY_CHIPS) - 1:
                row.addStretch()
