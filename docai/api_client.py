...
def _call(method: str, path: str, json_body: dict | None = None,
    ...
    if r.status_code >= 400 or not body.get("ok", False):
        raise ApiError(f"Máy chủ báo lỗi ({r.status_code}).",
...
def chat(message: str, file_name: str = "", file_type: str = "",
         business: dict | None = None, history: list | None = None) -> dict:
...
def edit_file(message: str, file_name: str, file_type: str,
              outline: str = "", history: list | None = None) -> dict:
    """Hỏi AI danh sách lệnh sửa file (chỉ word/excel).

    `outline` là cấu trúc tài liệu (đoạn/trang) để AI nhắm đúng vị trí.
